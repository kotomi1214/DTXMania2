    ...
    partial class 難易度
    {
        class 数値 : IDisposable
        {
            ...
            public void 開始する( double 数値 )
            {
                ...
                // 0.0 へ
                using( var 遷移 = Global.Animation.TrasitionLibrary.AccelerateDecelerate( duration: 難易度._アニメ時間sec / 2, finalValue: 0.0, accelerationRatio: 0.2, decelerationRatio: 0.8 ) )
                    this._ストーリーボード.AddTransition( this._左位置dpx, 遷移 );
                //----------------
                #endregion
            ...
            public void 進行描画する( DeviceContext d2ddc, float x, float y )
            {
                ...
                // 整数部を描画する（'.'含む）
                this._数字画像.描画する( d2ddc, 左位置dpx, y, this._難易度値文字列_整数部, new Size2F( 1.0f, 1.2f ) );

                // 小数部を描画する
                this._数字画像.描画する( d2ddc, 左位置dpx + 127f, y + 17f, this._難易度値文字列_小数部, new Size2F( 1.0f, 1.0f ) );
            }

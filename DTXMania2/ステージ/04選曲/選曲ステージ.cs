...
namespace DTXMania2.選曲
{
    class 選曲ステージ : IStage
    {
        ...
        public void 描画する()
        {
            ...
            switch( this.現在のフェーズ )
            {
                ...
                case フェーズ.QuickConfig:
                {
                    ...
                    this._背景画面を描画する( d2ddc );
                    this._システム情報.描画する( d2ddc );
                    this._QuickConfig画面.描画する( d2ddc, 568f, 68f );
        ...
        private void _その他パネルを描画する( DeviceContext d2ddc )
        {
            var preBlend = d2ddc.PrimitiveBlend;

            d2ddc.PrimitiveBlend = PrimitiveBlend.SourceOver;

            using( var ソートタブ上色 = new SolidColorBrush( d2ddc, new Color4( 0xFF121212 ) ) )
            using( var ソートタブ下色 = new SolidColorBrush( d2ddc, new Color4( 0xFF1f1f1f ) ) )
            {
                // 曲リストソートタブ
                d2ddc.FillRectangle( new RectangleF( 927f, 50f, 993f, 138f ), ソートタブ上色 );
                d2ddc.FillRectangle( new RectangleF( 927f, 142f, 993f, 46f ), ソートタブ下色 );
            }

            using( var 黒 = new SolidColorBrush( d2ddc, Color4.Black ) )
            using( var 白 = new SolidColorBrush( d2ddc, Color4.White ) )
            using( var 黒透過 = new SolidColorBrush( d2ddc, new Color4( Color3.Black, 0.5f ) ) )
            using( var 灰透過 = new SolidColorBrush( d2ddc, new Color4( 0x80535353 ) ) )
            {
                // インフォメーションバー
                d2ddc.FillRectangle( new RectangleF( 0f, 0f, 1920f, 50f ), 黒 );
                d2ddc.DrawLine( new Vector2( 0f, 50f ), new Vector2( 1920f, 50f ), 白, strokeWidth: 1f );

                // ボトムバー
                d2ddc.FillRectangle( new RectangleF( 0f, 1080f - 43f, 1920f, 1080f ), 黒 );

                // プレビュー領域
                d2ddc.FillRectangle( new RectangleF( 0f, 52f, 927f, 476f ), 黒透過 );
                d2ddc.DrawRectangle( new RectangleF( 0f, 52f, 927f, 476f ), 灰透過, strokeWidth: 1f );
        ...
        private void _背景画面を描画する( DeviceContext d2ddc )
        {
            ...
            if( 1 >= 曲ツリー.フォーカスリスト.Count )  // どのリストにも、最低減ランダムセレクトノードがある。
            {
                ...
                this._表示方法選択パネル.進行描画する( d2ddc );
                this._ステージタイマー.描画する( d2ddc, 1689f, 37f );
                this._SongNotFound.描画する( d2ddc, 1150f, 400f );
            }
            else
            {
                // (B) ノードがある場合 → 通常の画面
                ...
                this._舞台画像.進行描画する( d2ddc );
                this._選曲リスト.進行描画する( d2ddc );
                this._その他パネルを描画する( d2ddc );
                this._表示方法選択パネル.進行描画する( d2ddc );
                this._難易度と成績.進行描画する( d2ddc, 曲ツリー.フォーカス難易度レベル, 曲ツリー.フォーカスノード! );
                this._曲ステータスパネル.進行描画する( d2ddc, 曲ツリー.フォーカスノード! );
                this._プレビュー画像を描画する( d2ddc, 曲ツリー.フォーカスノード! );
                this._BPMパネル.進行描画する( d2ddc, 曲ツリー.フォーカスノード! );
                this._曲別スキルと達成率.進行描画する( d2ddc, 曲ツリー.フォーカスノード! );
                this._選択曲を囲む枠を描画する( d2ddc );
                this._選択曲枠ランナー.進行描画する( d2ddc );
                this._導線を描画する( d2ddc );
                this._ステージタイマー.描画する( d2ddc, 1689f, 37f );
                this._スクロールバーを描画する( d2ddc, 曲ツリー.フォーカスリスト );
                this._UpdatingSoglistパネル.進行描画する( d2ddc, 40f, 740f );
            }
        }

        private void _選択曲を囲む枠を描画する( DeviceContext d2ddc )
        {
            var 矩形 = new RectangleF( 1015f, 485f, 905f, 113f );

            this._青い線.描画する( d2ddc, new Vector2( 矩形.Left - _青枠のマージンdpx, 矩形.Top ), 幅dpx: 矩形.Width + _青枠のマージンdpx * 2f );
            this._青い線.描画する( d2ddc, new Vector2( 矩形.Left - _青枠のマージンdpx, 矩形.Bottom ), 幅dpx: 矩形.Width + _青枠のマージンdpx * 2f );
            this._青い線.描画する( d2ddc, new Vector2( 矩形.Left, 矩形.Top - _青枠のマージンdpx ), 高さdpx: 矩形.Height + _青枠のマージンdpx * 2f );
        }

        private void _スクロールバーを描画する( DeviceContext d2ddc, SelectableList<Node> フォーカスリスト )
        {
            int 曲数 = フォーカスリスト.Count;

            if( 2 > 曲数 )
                return; // 1曲しかないなら表示しない。

            var 全矩形 = new RectangleF( 1901f, 231f, 9f, 732f );  // 枠線含まず

            using var スクロールバー背景色 = new SolidColorBrush( d2ddc, new Color4( 0.2f, 0.2f, 0.2f, 1.0f ) );
            using var スクロールバー枠色 = new SolidColorBrush( d2ddc, Color4.Black );
            using var スクロールバーつまみ色 = new SolidColorBrush( d2ddc, Color4.White );

            d2ddc.DrawRectangle( 全矩形, スクロールバー枠色, 4f );
            d2ddc.FillRectangle( 全矩形, スクロールバー背景色 );
            ...
            d2ddc.FillRectangle( つまみ矩形, スクロールバーつまみ色 );
        }
        ...
        private void _プレビュー画像を描画する( DeviceContext d2ddc, Node フォーカスノード )
        {
            画像D2D image =
                ( !フォーカスノード.現行化済み ) ? this._現行化前のノード画像 :
                ( フォーカスノード.ノード画像 is null ) ? this._既定のノード画像 :
                フォーカスノード.ノード画像;
        ...
        private void _導線を描画する( DeviceContext d2ddc )
        {
            var h = (float)this._上に伸びる導線の長さdpx.Value;
            this._青い線.描画する( d2ddc, new Vector2( 1044f, 485f - h ), 高さdpx: h );

            var w = (float)this._左に伸びる導線の長さdpx.Value;
            this._青い線.描画する( d2ddc, new Vector2( 1046f - w, 278f ), 幅dpx: w );
            ...
            this._青い線.描画する( d2ddc, new Vector2( 右 + _青枠のマージンdpx - z, 上 ), 幅dpx: z ); // 上辺
            this._青い線.描画する( d2ddc, new Vector2( 右 + _青枠のマージンdpx - z, 下 ), 幅dpx: z ); // 下辺
            this._青い線.描画する( d2ddc, new Vector2( 左, 下 + _青枠のマージンdpx - z ), 高さdpx: z ); // 左辺
            this._青い線.描画する( d2ddc, new Vector2( 右, 下 + _青枠のマージンdpx - z ), 高さdpx: z ); // 右辺
        }
    }
}

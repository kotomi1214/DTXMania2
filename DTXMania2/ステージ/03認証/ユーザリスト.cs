    ...
    class ユーザリスト : IDisposable
    {
        ...
        public void 進行描画する( DeviceContext d2ddc )
        {
            ...
            for( int i = 0; i < 表示人数; i++ )
            {
                ...
                if( i == this.選択中のユーザ )
                    this._ユーザパネル光彩付き.描画する( d2ddc, 描画位置.X, 描画位置.Y + リストの改行幅 * i, 不透明度0to1: 不透明度 );
                this._ユーザパネル.描画する( d2ddc, 描画位置.X, 描画位置.Y + リストの改行幅 * i );

                // ユーザ名
                this._ユーザ名[ i ].描画する( d2ddc, 描画位置.X + 32f, 描画位置.Y + 40f + リストの改行幅 * i );

                // 肩書き
                this._ユーザ肩書きパネル.描画する( d2ddc, 描画位置.X, 描画位置.Y + リストの改行幅 * i );
            }
            //----------------
            #endregion
        }

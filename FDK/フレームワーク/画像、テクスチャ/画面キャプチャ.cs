...
namespace FDK
{
    public class 画面キャプチャ
    {
        ...
        {
            Debug.Assert( null != DXResources.Instance.D3D11Device1, "D3DDevice が取得されていません。" );

            // バックバッファの情報を取得する。
            var backBufferDesc = new Texture2DDescription();
            using( var backBuffer = DXResources.Instance.DXGISwapChain1.GetBackBuffer<Texture2D>( 0 ) )
            {
            ...
            using( var resource = DXResources.Instance.既定のD3D11RenderTargetView.Resource )
            {
                DXResources.Instance.D3D11Device1.ImmediateContext.CopyResource( resource, captureTexture );
            }
            ...
            using( var dxgiSurface = captureTexture.QueryInterface<Surface>() )
            {
                ...
                bitmap = new Bitmap(
                    ...
                    new BitmapProperties(
                        ...
                        DXResources.Instance.既定のD2D1DeviceContext.DotsPerInch.Width,
                        DXResources.Instance.既定のD2D1DeviceContext.DotsPerInch.Width ) );

    ...
    class ドラムサウンド : IDisposable
    {
        ...
        private void _登録する( チップ種別 chipType, int subChipId, VariablePath サウンドファイルパス )
        {
            ...
            lock( this._Sound利用権 )
            {
                ...
                context.SampleSource = SampleSourceFactory.Create( App進行描画.サウンドデバイス, サウンドファイルパス, 1.0 ); // ドラムサウンドは常に 1.0
                ...
                for( int i = 0; i < context.Sounds.Length; i++ )
                    context.Sounds[ i ] = new Sound( App進行描画.サウンドデバイス, context.SampleSource );

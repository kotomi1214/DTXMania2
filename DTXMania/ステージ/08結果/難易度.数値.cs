        ...
        class 数値 : IDisposable
        {
            ...
            public void 開始する( double 数値 )
            {
                ...
                if( this._refAnimation.TryGetTarget( out var animation ) )
                {
                    ...
                    // 0.0 へ
                    using( var 遷移 = animation.TrasitionLibrary.AccelerateDecelerate( duration: 難易度.アニメ時間sec / 2, finalValue: 0.0, accelerationRatio: 0.2, decelerationRatio: 0.8 ) )
                        this._ストーリーボード.AddTransition( this._左位置dpx, 遷移 );
                    ...
                    // 1.0 へ
                    using( var 遷移 = animation.TrasitionLibrary.Linear( duration: 難易度.アニメ時間sec, finalValue: 1.0 ) )
                        this._ストーリーボード.AddTransition( this._不透明度, 遷移 );
            ...
            public void 進行描画する( DeviceContext dc, float left, float top )
            {
                ...
                // 整数部を描画する（'.'含む）
                this._数字画像.描画する( dc, 左位置dpx, top, this._難易度値文字列_整数部, new Size2F( 1.0f, 1.2f ) );

                // 小数部を描画する
                this._数字画像.描画する( dc, 左位置dpx + 127f, top + 17f, this._難易度値文字列_小数部, new Size2F( 1.0f, 1.0f ) );
            }

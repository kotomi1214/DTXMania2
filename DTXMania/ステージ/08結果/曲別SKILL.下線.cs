        ...
        class 下線 : IDisposable
        {
            ...
            public void 開始する()
            {
                if( this._refAnimation.TryGetTarget( out var animation ) )
                {
                    ...
                    // 全長dpx へ
                    using( var 遷移 = animation.TrasitionLibrary.AccelerateDecelerate( duration: 曲別SKILL.アニメ時間sec / 3, finalValue: _全長dpx, accelerationRatio: 0.8, decelerationRatio: 0.2 ) )
                        this._ストーリーボード.AddTransition( this._長さdpx, 遷移 );

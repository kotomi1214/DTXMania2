    ...
    partial class 難易度
    {
        class アイコン : IDisposable
        {
            ...
            public アイコン( Animation animation )
            {
                ...
                this._アイコン画像 = new テクスチャ( @"$(System)Images\結果\難易度アイコン.png" );
                this._ストーリーボード = null;
                this._拡大角度rad = null;
                this._半径倍率 = null;
                this._不透明度 = null;
            ...
            public void 開始する()
            {
                if( this._refAnimation.TryGetTarget( out var animation ) )
                {
                    ...
                    // 2π へ
                    using( var 遷移 = animation.TrasitionLibrary.Linear( duration: 難易度.アニメ時間sec, finalValue: 2 * Math.PI ) )
                        this._ストーリーボード.AddTransition( this._拡大角度rad, 遷移 );
                    ...
                    // 0.0 へ
                    using( var 遷移 = animation.TrasitionLibrary.Linear( duration: 難易度.アニメ時間sec, finalValue: 0.0 ) )
                        this._ストーリーボード.AddTransition( this._半径倍率, 遷移 );

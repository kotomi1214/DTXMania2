...
namespace DTXMania.オプション設定
{
    partial class 曲読み込みフォルダ割り当てダイアログ : Form
    {
            ...
            : base()
        {
            ...
            foreach( var vpath in 現在の曲検索フォルダリスト )
        ...
        private void 曲読み込みフォルダ割り当てダイアログ_FormClosing( object sender, FormClosingEventArgs e )
        {
            if( this.DialogResult == DialogResult.Cancel )    // ウィンドウを閉じようとした時も Cancel になる。
            {
                if( this._リストに変更がある )
                {
                    if( MessageBoxEx.Show( "変更を破棄していいですか？", "確認", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button2 ) == DialogResult.No )
                        e.Cancel = true;
                }
            }
        }

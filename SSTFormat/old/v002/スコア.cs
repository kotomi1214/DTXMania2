...
namespace SSTFormat.v002
{
    public class スコア : IDisposable, ISSTFScore
    {
        ...
        public SSTFVersion SSTFVersion { get; } = new SSTFVersion( 2, 0, 0, 0 );
        ...
        {
            public SSTFVersion SSTFバージョン = new SSTFVersion( 1, 0, 0, 0 );   // SSTFVersion 指定がない場合の既定値。
        ...
        public スコア()
        {
            this.Header.SSTFバージョン = new SSTFVersion( 2, 0, 0, 0 );  // このソースで対応するバージョン
            this.チップリスト = new List<チップ>();
            this.小節長倍率リスト = new List<double>();
        }

        public スコア( string 曲データファイル名, bool ヘッダだけ = false )
            : this()
        {
            if( ヘッダだけ )
                this.曲データファイルを読み込む_ヘッダだけ( 曲データファイル名 );
            else
                this.曲データファイルを読み込む( 曲データファイル名 );
        }

        public スコア( v001_2.スコア v1_2score )
        {
            this.Header = new CHeader() {
                曲名 = v1_2score.Header.曲名,
                説明文 = v1_2score.Header.説明文,
                サウンドデバイス遅延ms = v1_2score.Header.サウンドデバイス遅延ms,
            };

            this.背景動画ファイル名 = v1_2score.背景動画ファイル名;
            this.dicメモ = v1_2score.dicメモ;
            this.小節長倍率リスト = v1_2score.小節長倍率リスト;
            this.チップリスト = new List<チップ>();
            foreach( var chip in v1_2score.チップリスト )
                this.チップリスト.Add( new チップ( chip ) );
        }
        ...
        public void 曲データファイルを読み込む( string 曲データファイル名 )
        {
            ...
            this.Header.譜面ファイルパス = 曲データファイル名;
            //-----------------
            #endregion
            ...
            using( var sr = new StreamReader( 曲データファイル名, Encoding.UTF8 ) )
            {
                ...
                while( false == sr.EndOfStream )
                {
                        ...
                        行.StartsWith( "SSTFVersion", StringComparison.OrdinalIgnoreCase ) )
                    {
                        ...
                        try
                        {
                            this.Header.SSTFバージョン = new SSTFVersion( items[ 1 ].Trim() );   // string から Version へ変換できる書式であること。（例: "1.2.3.4"）
                        }
                        catch
                        {

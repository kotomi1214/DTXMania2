...
namespace FDK32
{
    ...
    public class GraphicsContext : IDisposable
    {
        ...
        /// </summary>
        /// <remarks>
        ///     描画先コントロールに対して１つずつインスタンスを生成することができる。
        /// </remarks>
        public static Dictionary<IntPtr, GraphicsContext> Instance { get; protected set; } = new Dictionary<IntPtr, GraphicsContext>();

        /// <summary>
        ///     <see cref="Instance"/> を生成する。
        /// </summary>
        /// <param name="hWindow">ウィンドウハンドル。DirectComposition で使用する。</param>
        ...
        public static void 生成する( Size 物理画面サイズ, Size 設計画面サイズ, IntPtr hWindow, IntPtr hControl )
        {
            using( Log.Block( FDK32.FDKUtilities.現在のメソッド名 ) )
            {
                ...
                Instance[ hControl ] = new GraphicsContext( 物理画面サイズ, 設計画面サイズ, hWindow, hControl );
            }
        }

        /// <summary>
        ///     <see cref="Instance"/> を終了する。
        /// </summary>
        ...
        protected GraphicsContext( Size 物理画面サイズ, Size 設計画面サイズ, IntPtr hWindow, IntPtr hControl )
        {
        ...
        /// </remarks>
        public void 平面描画用の変換行列を取得する( float 視野角deg, out Matrix 転置済みビュー行列, out Matrix 転置済み射影行列 )
        {
